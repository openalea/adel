...
from openalea.adel.colormap import colormap
from openalea.adel.geometric_elements import Leaves
from openalea.adel.Stand import AgronomicStand
from openalea.adel.mtg_interpreter import plot3d, transform_geom, mtg_interpreter
from openalea.adel.postprocessing import (
    axis_statistics,
    plot_statistics,
    midrib_statistics,
)
...
def balanced_sample(n, proba):
    """return a list of n keys found in proba, respecting probabilities of proba values"""
...
class Adel:
    ...
    def __init__(
        ...
        Args:
            ...
            species: a {species: frequency} dict indicating the composition of
             the canopy. If None (default), a mono specific canopy of species '0'
             is generated
        ...
        self.nrem = None
        self.plant_references = None
        self.plant_species = None
        self.plant_azimuths = None
        ...
        if convUnit is not None:
            warnings.warn(
                ...
                " use scene_unit=unit_name (" + ",".join(self.conv_units) + ") instead",
                DeprecationWarning,
            )
        ...
        if stand is None:
            stand = AgronomicStand(
                sowing_density=250, plant_density=250, inter_row=0.15
            )
    ...
    def new_stand(
        ...
        if self.aspect == "smart":
            self.nplants, self.domain, self.positions, self.domain_area = (
                self.stand.smart_stand(
                    self.nplants, at=age, convunit=1.0 / self.convUnit
                )
            )
        else:
            self.nplants, self.domain, self.positions, self.domain_area = (
                self.stand.stand(
                    self.nplants, aspect=self.aspect, convunit=1.0 / self.convUnit
                )
            )

        self.plant_azimuths = numpy.random.random(self.nplants) * 360
        self.plant_species = balanced_sample(self.nplants, self.species)
        self.plant_references = numpy.random.choice(
            list(range(self.nref_plants)), self.nplants
        )

        stand_parameters = {
            "sowing_density": self.stand.sowing_density,
    ...
    def get_midribs(self, g, resample=False):
        ...
        blades = (
            vid
            for vid in g.vertices(scale=g.max_scale() - 1)
            if g.label(vid).startswith("blade")
        )
        ...
        midribs = {
            vid: self.leaves[species[vid]].midrib(g.node(vid), resample=resample)
            for vid in species
        }
        #
        anchor = g.property("anchor_point")
        midribs_anchor = {
            vid: [anchor[cid] for cid in g.components(vid) if cid in anchor]
            for vid in midribs
        }
        hins = {
            k: v[0][2] + midribs[k][2] for k, v in midribs_anchor.items() if len(v) > 0
        }
        ...
        res = [
            ...
            for vid in hins
        ]  # hins keys are for midribs keys which also have a geometry (anchor point
