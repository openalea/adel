...
class AdelWheat(Adel):
    def __init__(
        ...
        """
        Args:
             devT: adel parameter, eg as generated by plantgen
             devT_unit: lenght unit used in devT (eg 'cm')
             sample: if 'random' (default) plants are sampled randomly in devT, otherwise devT is recycled keeping its original order
             thermal_time_model: a callable computing thermal time
             geoAxe: a R code to pass to adel for axe geometry. If none, built from incT, dinT and dep
             incT: basal inclination of tillers (deg)
             dinT: the random deviation around incT
             dep: the distance (cm) at which a tiller ear is far from rank at maturity
             run_adel_pars: a dict of inner parameter for adel
             aborting_tiller_reduction: a reduction factor for dimension, applied gobally to devT
             ssipars: a set of parameter for senescence
             *args, **kwargs: parameters passed to parent class: Adel
        """
        ...
        self.ref_plants = list(set(devT["axeT"]["id_plt"]))
        kwargs.update({'nref_plants': len(self.ref_plants)})

        super(AdelWheat, self).__init__(*args, **kwargs)

        self.canopy_age = None

        if devT_unit != self.scene_unit:
            convert = self.conv_units[devT_unit] / self.conv_units[self.scene_unit]
            for x in (
                "L_internode",
                "W_internode",
                "W_sheath",
        ...
        if run_adel_pars is None:
            run_adel_pars = {
                ...
                "dHS_col": 0.2,
                "dHS_en": 0,
                "epsillon": 1e-6 * self.conv_units["m"] / self.conv_units[self.scene_unit],
        ...
        if geoAxe is None:
            cm_to_scene_unit = self.conv_units['cm'] / self.conv_units[self.scene_unit]
            dep *= cm_to_scene_unit
            depMin = 1.5 * cm_to_scene_unit
            geoAxe = genGeoAxe(incT=incT, dinT=dinT, dep=dep, depMin=depMin)
    ...
    def run_adel(self, age=10, as_df=False):
        if self.duplicate is None:
            ...
            if as_df:
                return pandas.DataFrame(canopy_quot), pandas.DataFrame(canopy_rem)
            else:
